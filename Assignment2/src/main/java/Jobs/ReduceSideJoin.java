...
//        job.setMapperClass(ReduceSideJoin.MapClass.class);
        job.setPartitionerClass(ReduceSideJoin.PartitionerClass.class);
        job.setReducerClass(ReduceSideJoin.ReduceClass.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(Text.class);
        MultipleInputs.addInputPath(job, new Path(inputPath1), InputFormat_N3.class, ReduceSideJoin.MapClass.class);
        MultipleInputs.addInputPath(job, new Path(inputPath2), formatToJoin.getClass(), ReduceSideJoin.MapClass.class);
    ...
    public static Job[] createJoinTable() {
        ...
        try {
            job_join_N1 = CreateJoinJob(Constants.JOB_JOIN_N1, Constants.JOIN_OUTPUT, Constants.OCC_3_GRAMS, Constants.CORPUS_1_GRAMS, new InputFormat_N1());
        }
        catch (IOException e) {
            e.printStackTrace();
        }
